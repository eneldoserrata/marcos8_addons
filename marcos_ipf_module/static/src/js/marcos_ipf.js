...
openerp.marcos_ipf_module = function (instance, local) {
    ...
    local.ipfAPI = instance.Widget.extend({
        ...
        get_z_close_range: function(date_from, date_to, context) {
            var self = this;
            var from = date_from.split("-");
            var to = date_to.split("-");
            openerp.web.blockUI();
            self.get_host(context).then(function (context) {
                var url = self.host + "/zclose/range/"+from[2]+from[1]+from[0]+"/"+to[2]+to[1]+to[0];
                return self.get_report(url, "GET", "get_z_close_print");
            }).then(function(){
                openerp.web.blockUI();
            });
    ...
    local.IpfPrinterZCloseRange = instance.web.form.FormWidget.extend({
        events: {
            "click #get_z_close_range": "get_z_close_range"
        },
        start: function () {
            var self = this;
            this.$el.append("<button id='get_z_close_range' class='oe_inline oe_stat_button'><div class='stat_button_icon fa fa-calendar'></div><div>Cierre Z</div></button>");
        },

        get_z_close_range: function () {
            var self = this;
            if (!checkBrowserCompatibility()){
                return
            }
            ...
            var zrange_form="";
                zrange_form += "<form class=\"form-horizontal\">";
                zrange_form += "";
                zrange_form += "<div class=\"form-group\">";
                zrange_form += "  <label class=\"col-md-4 control-label\" for=\"textinput\">Desde<\/label>  ";
                zrange_form += "  <div class=\"col-md-4\">";
                zrange_form += "  <input id=\"zrange_date_from\" name=\"textinput\" type=\"date\" placeholder=\"placeholder\" class=\"form-control input-md\" requiered>";
                zrange_form += "  <\/div>";
                zrange_form += "<\/div>";
                zrange_form += "";
                zrange_form += "<div class=\"form-group\">";
                zrange_form += "  <label class=\"col-md-4 control-label\" for=\"textinput\">Hasta<\/label>  ";
                zrange_form += "  <div class=\"col-md-4\">";
                zrange_form += "  <input id=\"zrange_date_to\" name=\"textinput\" type=\"date\" placeholder=\"placeholder\" class=\"form-control input-md\" requiered>";
                zrange_form += "  <\/div>";
                zrange_form += "<\/div>";
                zrange_form += "";
                zrange_form += "<\/form>";
            ...
            bootbox.dialog({
                    title: "Imprimir cierre Z por rango de fecha.",
                    message: zrange_form,
                    buttons: {
                        success: {
                            label: "Imprimir",
                            ...
                            callback: function () {
                                ...
                                var command = new local.ipfAPI();
                                var zrange_date_from = $("#zrange_date_from").val();
                                var zrange_date_to = $("#zrange_date_to").val();
                                command.get_z_close_range(zrange_date_from, zrange_date_to, context);
                            }
                        }
                    }
                }
    ...
    openerp.web.form.custom_widgets.add("ipf_button_z_close_range", "instance.marcos_ipf_module.IpfPrinterZCloseRange");
